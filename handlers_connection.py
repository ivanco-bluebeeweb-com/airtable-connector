"""Connection lifecycle for Airtable Connector."""
...
@chat.function(
    "connect_airtable",
    "Connect Airtable account via credentials.",
    action_type="write",
    chain_callable=True,
    event="airtable-connector.connect_airtable",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_airtable(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Airtable Connector."""
    client = AirtableClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Airtable Account",
...
@chat.function(
    "list_connections",
    "List connected Airtable accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    ...
    action_type="write",
    chain_callable=True,
    event="airtable-connector.disconnect_airtable",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_airtable(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
